...
public abstract class RestClient {
    ...
    private static final String REPORT_SCOPE_SCAN = "Scan"
    private static final String API_CREATE_SECURITY_REPORT = "/api/V4/Reports/Security/%s/%s"
    private static final String API_DOWNLOAD_REPORT = "/api/V4/Reports/%s/Download"
    private static final int REPORT_DOWNLOAD_MAX_ATTEMPTS = 60
    private static final int REPORT_DOWNLOAD_RETRY_INTERVAL_SECONDS = 5
	...
	public RestClient(Properties props) {
		String validateSSLProperty = props.containsKey("validateSSL") ? String.valueOf(props["validateSSL"]) : "true"
		this.validateSSL = Boolean.parseBoolean(validateSSLProperty)
		boolean trustAllCerts = !this.validateSSL
		if (!this.validateSSL) {
    ...
    private String uploadFile(File file) {
        ...
        // ASoC v4 upload endpoint expects the multipart form field name `uploadedFile`.
    ...
    public String startStaticScan(File arsaFile, String parentjobid, String appId, boolean mailNotification) {
        ...
        // v4 SAST endpoint expects ApplicationFileId as the source-of-code identifier.
    ...
    private void downloadSingleReportType(String scanId, String reportType) {
        String createReportUrl = String.format(API_CREATE_SECURITY_REPORT, REPORT_SCOPE_SCAN, scanId)
        String createReportBody = new JsonBuilder([Configuration: [ReportFileType: reportType]]).toString()

        println("[Action] Sending POST request to ${this.baseUrl}$createReportUrl for ${reportType} report creation.")
        def createResponse = restHelper.doPostRequest(createReportUrl, createReportBody)
        def reportStatus = restHelper.parseResponse(createResponse)

        String reportId = reportStatus?.Id
        if (reportId == null || reportId.toString().trim().isEmpty()) {
            println("[Error] Failed creating ${reportType} report. Missing report id in response: ${reportStatus}")
            System.exit(1)
        }

        String downloadUrl = reportStatus?.DownloadLink ?: String.format(API_DOWNLOAD_REPORT, reportId)

        String logUrl = "${this.baseUrl}${downloadUrl.startsWith('http') ? '' : downloadUrl}"
        def response = null

        for (int attempt = 1; attempt <= REPORT_DOWNLOAD_MAX_ATTEMPTS; attempt++) {
            println("[Action] Sending GET request to ${logUrl}. Attempt ${attempt}/${REPORT_DOWNLOAD_MAX_ATTEMPTS}.")

            try {
                response = restHelper.doGetRequest(downloadUrl)
                break
            }
            catch (Exception ex) {
                String message = ex.getMessage() ?: ""
                boolean reportNotReady = message.contains("REPORT_IS_NOT_AVAILABLE")

                if (!reportNotReady || attempt == REPORT_DOWNLOAD_MAX_ATTEMPTS) {
                    throw ex
                }

                println("[Warning] ${reportType} report is not available yet. Retrying in ${REPORT_DOWNLOAD_RETRY_INTERVAL_SECONDS} seconds.")
                Thread.sleep(TimeUnit.SECONDS.toMillis(REPORT_DOWNLOAD_RETRY_INTERVAL_SECONDS))
            }
        }

        if (response == null) {
            println("[Error] Failed downloading ${reportType} report. No response was received.")
            System.exit(1)
        }
